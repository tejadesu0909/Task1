...
const EmployeeComponent = () => {
  ...
  //     if (validateForm()) {
  //       const updatedFields = {};
  //       if (firstName.trim()) updatedFields.firstName = firstName;
  //       if (lastName.trim()) updatedFields.lastName = lastName;
  //       if (technology.trim()) updatedFields.technology = technology;
  //       if(yearsOfExperience) updatedFields.yearsOfExperience = yearsOfExperience
  //       if (salary) updatedFields.salary = salary
  //       if (location.trim()) updatedFields.location = location
  //       if (email.trim()) updatedFields.email = email
  //       if (phoneNumber) updatedFields.phoneNumber = phoneNumber
  //       if (employmentType.trim()) updatedFields.employmentType = employmentType

  //       console.log("Updating Employees with fields",updatedFields);
  ...
  async function saveorUpdateEmployee(e) {
    ...
    if (validateForm()) {
      ...
      if (yearsOfExperience)
        updatedFields.yearsOfExperience = yearsOfExperience;
      if (salary) updatedFields.salary = salary;
      if (location.trim()) updatedFields.location = location;
      if (email.trim()) updatedFields.email = email;
      if (phoneNumber) updatedFields.phoneNumber = phoneNumber;
      ...
      if (Object.keys(updatedFields).length === 0) {
        console.warn("No changes detected. Skipping API request.");
        return;
      }
      ...
      try {
        let response;
        if (id) {
          response = await updateEmployee(id, updatedFields);
          console.log("Update Response:", response.data);
        } else {
          response = await createEmployee(updatedFields);
          console.log("Create Response:", response.data);
        }

        navigator("/employees");
      } catch (error) {
        console.error("Error in saveorUpdateEmployee:", error);
